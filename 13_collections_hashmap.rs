...
fn main() {
    ...
    // 练习1：
    // 步骤1: 设计数据结构
    // Key 是 String 类型的部门名, Value 是 Vec<String> 类型的员工列表
    let mut company: HashMap<String, Vec<String>> = HashMap::new();
    println!("Welcome to the Company Roster Management System!");
    println!("Commands:");
    ...
    loop {
        ...
        match words.get(0) {
            Some(&"Add") => {
                // 期望的格式: "Add" <name> "to" <department>
                if words.len() == 4 && words[2].eq_ignore_ascii_case("to") {
                    let name = words[1].to_string();
                    let department = words[3].to_string();
                    
                    // 步骤4: 添加员工
                    // entry API 检查部门是否存在，如果不存在则插入一个新的空 Vec [1][2]
                    let employees = company.entry(department.clone()).or_insert(vec![]);
                    employees.push(name);
                    println!("Added {} to {}", words[1], department);
                } else {
                    println!("Invalid format. Please use: Add <name> to <department>");
                }
            }
            Some(&"List") => {
                // 期望的格式: "List" <department>
                if words.len() == 2 {
                    let department = words[1];
                    
                    // 步骤5: 查询部门并排序打印
                    // 使用 get 方法从 HashMap 中访问值 [1][2]
                    ...
                    println!("Invalid format. Please use: List <department>");
                }
            }
            Some(&"Exit") => {
